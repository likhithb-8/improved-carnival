...
## 2024-06-17 - Permissions Policy and Local SRI Trade-offs
**Vulnerability:** Unintended access to browser features and maintenance burden of local SRI.
**Learning:** Implementing a Permissions-Policy provides defense-in-depth by disabling unused browser features like camera and microphone. Conversely, hardcoded SRI for local assets is an anti-pattern as it breaks the UI on every style change without providing significant security benefits over repo-level integrity.
**Prevention:** Use Permissions-Policy to reduce attack surface and avoid SRI on local assets in high-velocity development environments.
